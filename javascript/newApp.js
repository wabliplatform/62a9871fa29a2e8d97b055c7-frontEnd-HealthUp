let apiAppApi = new TempApi.AppApi();import TempApi from '../src/index';let app = new TempApi.App();document.getElementById('il5w').onclick = (event) => {
...
document.getElementById('iglgul').onclick = (event) => {
    event.preventDefault();
    app['appTitle'] = document.querySelector("[annotationname = 'appTitle']").value;app['appDescription'] = document.querySelector("[annotationname = 'appDescription']").value;app['appImage'] = {
      ...
      apiAppApi.createapp( app, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); {  location.href= '/AppList/'+response.body.query._id+'' ;}}});};window.onload = () => {};